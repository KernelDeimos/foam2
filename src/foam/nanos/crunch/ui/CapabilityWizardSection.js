  ...
  properties: [
    ...
    },
    {
      ...
      expression: function (ucj) {
        if ( ucj === null || ucj.data === null ) {
          return this.of.create({}, this);
        }
        console.log('data-before-send', ucj.data);
        return ucj.data.clone();
      }
    },
    {
      name: 'debuginfo',
      expression: (capability, data) => {
        return `Section ID ${capability.id}
  data (hour): ${
    (data && data.duration) ? data.hour : 'none'
  }`;
      }
    }
  ],
